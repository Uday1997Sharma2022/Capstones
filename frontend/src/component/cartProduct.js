...
const CartProduct = ({ id, name, image, category, qty, total, price }) => {
  ...
  return (
    <div className='bg-slate-200 p-2 flex gap-4 rounded border border-slate-300'>
        ...
        <img src={image} className="h-30 w-20 object-cover " />
        </div>
        <div className='flex flex-col gap-1 w-full'>
        <div className="flex justify-between">
      <h3 className="font-semibold text-slate-600 capitalize text-lg md:text-xl">{name}</h3>
      <div className="cursor-pointer text-slate-700 hover:text-red-500" onClick={()=>dispatch(deleteCartItem(id))}><AiFillDelete /></div>
      </div>
      ...
      <div className="flex items-center gap-2 font-bold text-slate-700"><p>Total :</p><p><span className="text-red-500">₹</span>{total}</p></div>
      </div>
      </div>
    </div>
  )
